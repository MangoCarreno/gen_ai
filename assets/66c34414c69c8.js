...
/* Copyright (c) 2024 Elucidat Ltd - All rights reserved */
console.log("Elucidat.com Rapid Authoring (v.#build_version#) built with love on 2024-08-19 01:08:10");
/* 
notes - to add:
    1.2
        cmi.core.student_id
...
Elucidat = function (options, lrs_activity_id, lrs_endpoints) {
    ...
    // now navigation
    context._load_navigation(window.NAVIGATION_OBJECT);
    // get learner name
		...
		if ($.fn.popover && $.fn.modal.Constructor) {
			...
			$.fn.collapse.Constructor.prototype.toggle = function () {
				...
				if (!shown) {
					...
					$el.parents('.accordion:first').find('.accordion-heading > button').attr('aria-expanded', false);
					$el.siblings('.accordion-heading').find('> button').addClass('opened');
				} else {
					$el.siblings('.accordion-heading').find('> button').removeClass('opened');					
					$el.siblings('.accordion-heading').find('> button').attr('aria-expanded', false);		
				}
...
Elucidat.prototype._manage_progress = function () {
    ...
    */
    ...
    if (c.options.lms.mode) {
                ...
                if (c.options.completion_action == 'completed' || c.options.lms.mode != '1.2') {
                    if (completion_result) {
                        ...
                        setTimeout(function () {
                            ...
                            if(c.options.lms.mode == '2004' && global_pass.score_possible == 0) {
                                // If there are no scored questions then we should set the score to 100% and mark as passed..
                                c.options.lms.SetPassed();
                                c.options.lms.SetScore(0, 0, 100);
                            }
                        },50);
        ...
        if((none_scored && !c.options.enable_success_factors_support) || c.sent_result && c.sent_lms_completion){
        ...
        update_form_status: function(options) {
            ...
            return this.each(function() {
                ...
                if ( options.is_answered ) {
                    ...
                    // If we have intentionally added an aria label to this button, do not remove the aria label
                    if (!$save_button.data('aria-changed-label')) {
                        ...
                        if (video_object[0] == 'video' || video_object[0] == 'youtube' || video_object[0] == 'external') {
                            ...
                            if (video_object[0] == 'video' || video_object[0] == 'external') {
                                ...
                                if (video_url) {
                                    ...
                                    var isS3Video = basename.indexOf('s3up.');
                                    // add the poster image in if one has been specified


                                    if ($video_player.attr('data-poster')) {
                                        // if poster is defined
                                        var dataPoster = $video_player.attr('data-poster');

                                        if (dataPoster.indexOf('{{') > -1 && isS3Video) {
                                            poster = '';
                                        } else {
                                    ...
                                    if (video_object[0] == 'video') {
                                        ...
                                        if (poster === '') {
                                            if(isS3Video) {
                                                poster = basename.replace('.s3up','') + '.0000001.jpg';
                                            } else {
        ...
        graph_build: function(graphs, isPlaceholder) {
            ...
            return this.each(function() {
                ...
                for(var i = 0; i<graphs.length; i++) {
                    ...
                    var $canvas = $('<canvas id="' + chart_id + '" role="img"></canvas>');
                    ...
                    // At some point in the future it may be possible to declare aria-labels, so in preparation we check if there is an aria label
                    // eslint-disable-next-line prefer-template
                    var domAriaLabel = $canvas.closest('.chart_wrapper').attr('aria-label') + ': ' || "";
                    var ariaLabel = domAriaLabel + $.map( options.labels, function( val, index ) {
                        // eslint-disable-next-line prefer-template
                        return val + ': ' + options.data[index];
                    });
...
(function($){
            ...
            return this.each(function () {
                ...
                if (options === 'destroy') {
                    ...
                    if ($linkOrButton.attr('data-tooltip')) {
                        ...
                        // Take the string data and add it to the DOM so that screenreader knows that there is text in the tooltip and reads it out
                        $linkOrButton.attr('aria-label', json_data.title + ', ' + json_data.text);
    ...
    // Tooltips should close upon pressing the escape key,  as per:
    // https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Roles/tooltip_role#required_javascript_features
    function escKeyPressHideTooltip (event) {
        if (event.which === 27) {
            hideAllTooltips();
        }
    }
    $(document).on('shown.bs.tooltip', function () {
        document.addEventListener('keydown', escKeyPressHideTooltip);
    });

    $(document).on('hide.bs.tooltip', function () {
        document.removeEventListener('keydown', escKeyPressHideTooltip);
    });
...
(function($){
    $.fn.extend({
        accessibility_fixes: function(){
            ...
            // Several times in this file we change aria-labels on elements programatically. 
            // We have now made it possible to manually set aria-labels.
            // I want the aria-label that the user chooses to be respected and not overwritten by the JS.
            // When the user edits aria label in author view we add an 'aria changed label' flag to the DOM
            // so look to see if an aria changed label flag is present, and if it is respect that. 
            function applyAriaLabelIfNone ($element, label) {
                if (!$element.data('aria-changed-label')) {
            ...
            return this.each(function () {
                ...
                $this.find('.accordion-group').each(function() {
                    var $accordion = $(this);
                    var $accordionHeading = $accordion.find('button.accordion-heading');
                ...
                $this.find('.timeline__entry').each(function() {
                    ...
                    applyAriaLabelIfNone($timelineEntryButton, label);
                ...
                $this.find('.hotspot').each(function() {
                    ...
                    applyAriaLabelIfNone($hotspot, text);
                ...
                $('.explorer__tooltip .htmlText').each(function(){
                    var $this = $(this);
                    var $text = $(this).text().trim();
                    applyAriaLabelIfNone($this, $text);
                });
                        ...
                        if($anchor.hasClass('e-link-disabled-by-rule')){
                            ...
                            if($anchor.hasClass('menu__head__entry__inner')){
                                ...
                                if ( $anchor_text.is(':hidden') ) {
                                    applyAriaLabelIfNone($anchor, $anchor_text.text());
                                }
                            }
                        }
                    });
                ...
                // It is invalid HTML to have a <p> tag inside a <label> tag, so we need to replace the <p> with a <span> tag
                // We get p tags in the label because the tinyMCE editor adds them
                // TinyMCE doesn't add any attributes to the p tags, but on the off chance in future it does, we need to copy them over to the new spans
                $('label').find('p').each(function() {
...
window.NAVIGATION_OBJECT={"template":"<li class=\"menu__item {{page.is_section}} {{page.active}} e-no-controls\">\n    <a href=\"{{page.url}}\" class=\"itemInner menu__item__inner\" title=\"{{page.name}}\" data-dismiss=\"dropdown\">\n        <span class=\"icon\">\n            <i class=\"ti ti-lock\"><span class=\"focusHelper\"><\/span><\/i>\n        <\/span>\n        <span class=\"text e-no-controls\">{{page.name}}<\/span>\n    <\/a>\n    {{page.sub_pages}}\n<\/li>","url_format":"pages\/%s.js","pages":{"655ba56956de7":{"n":"Introduction to Generative AI and Large Language Models","mn":false,"mod":false,"hol":false,"sec":true,"i":false,"s":1,"b":"_6579d3f33ea83","v":"viewed","d":true},"655bbc486d7f2":{"n":"Introduction to Generative AI and Large Language Models","mn":false,"mod":false,"hol":false,"sec":false,"i":false,"b":"_6579cae072ec6","v":"viewed","p":"655ba56956de7"},"655ba5695d09f":{"n":"Concepts of Generative AI and Large Language Model","mn":false,"mod":false,"hol":false,"sec":false,"i":false,"b":"_65a69f60b400f","v":"viewed","p":"655ba56956de7"},"655bbc5ab6f50":{"n":"Stages and Scale of LLM Training","mn":false,"mod":false,"hol":false,"sec":false,"i":false,"b":"_65773d8e06749","v":"viewed","p":"655ba56956de7"},"655ba56960e06":{"n":"Knowledge Check: Introduction to Generative AI and Large Language Models","mn":false,"mod":false,"hol":false,"sec":false,"i":false,"b":"_655ba56960e42","v":"viewed","p":"655ba56956de7"},"655ba5696369d":{"n":"Question 1","mn":false,"mod":false,"hol":false,"sec":false,"i":false,"m":1,"c":1,"b":"_655ba569636d9","v":"any-score","p":"655ba56956de7"},"6579d2a56ed6c":{"n":"Question 2","mn":false,"mod":false,"hol":false,"sec":false,"i":false,"m":1,"c":1,"b":"_6579d2a56edac","v":"any-score","p":"655ba56956de7"},"6579d2d49ebfa":{"n":"Question 3","mn":false,"mod":false,"hol":false,"sec":false,"i":false,"m":1,"c":1,"b":"_6579d2d49ec3a","v":"any-score","p":"655ba56956de7"},"659c583ee2506":{"n":"Question 4","mn":false,"mod":false,"hol":false,"sec":false,"i":false,"m":1,"c":1,"b":"_659c583ee254c","v":"any-score","p":"655ba56956de7"},"659c59ca276d6":{"n":"Question 5","mn":false,"mod":false,"hol":false,"sec":false,"i":false,"m":1,"c":1,"b":"_659c5a08602b2","v":"any-score","p":"655ba56956de7"},"655ba5696842c":{"n":"Results","mn":false,"mod":false,"hol":false,"sec":false,"i":false,"m":1,"b":"_655ba56968469","v":"viewed","p":"655ba56956de7"}}}
$('body').on('elucidat.page.ready', function (event, page_data, $new_page) {
